...
namespace MedicApp.Infrastructure.FileManager
{
    ...
    public class FileManager : IFileManager
    {
        public void Delete(string filename, string deletedPath = "uploads")
        {
            string path = Path.Combine(
                                Directory.GetCurrentDirectory(), "resource", deletedPath,
                                filename);
            if (File.Exists(path))
            {
                File.Delete(path);
        ...
        public string Upload(IFormFile file, string savePath = "resource", string newName = null)
        {
            string filename;
            ...
            var writePath = Path.Combine(Directory.GetCurrentDirectory(), "resource", savePath);
